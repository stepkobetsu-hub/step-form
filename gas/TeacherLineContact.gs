...
function teacherLineContactAccessToken_() { const p=PropertiesService.getScriptProperties(); const token=p.getProperty('LINE_CHANNEL_ACCESS_TOKEN')||p.getProperty('LINE_ACCESS_TOKEN')||p.getProperty('CHANNEL_ACCESS_TOKEN'); if(!token)throw new Error('LINE送信用設定がありません。'); return token; }
